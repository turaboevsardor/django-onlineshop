...
bot = telebot.TeleBot('5215908134:AAENAcUVESBfs6svU8XmWI1n0wug0jom2hE')
...
def get_name(message):
    ...
    bot.send_message(user_id, 'Kinoning ismi')
...
def get_price(message):
    ...
    bot.send_message(user_id, 'Kino qoshildi')
...
def text(message):
    ...
    if tuple([message.text]) in o:
        ...
        product = datta.User().get_info(message.text)
    ...
    # если пользователь нажмет на кнопку корзина мв из базы достаем его товары
        ...
        t = datta.User().full_cart(user_id)
...
def get_quantity(message):
    ...
    datta.User().get_cart(user_cart['user_id'], user_cart['product_name'], user_cart['product_count'])
    bot.send_message(user_id, 'Bizni tanlaganingiz uchun rahmat ')
